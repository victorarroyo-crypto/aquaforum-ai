    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
          <div className="w-8 h-px bg-teal mx-auto mb-8" />
          <p className="text-ink-faint text-sm font-serif italic">Conectando al foro...</p>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="flex h-screen flex-col">
      ...
      <header className="flex items-center justify-between px-5 py-3 border-b border-rule bg-paper">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xs font-semibold uppercase tracking-widest text-ink">AquaForum AI</h1>
            <p className="max-w-sm truncate text-[11px] text-ink-faint">{topic}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 border border-rule rounded-full px-3 py-1">
            <span className="text-[10px] text-ink-ghost">Ronda</span>
            <span className="text-sm font-serif font-semibold text-teal">{currentRound}</span>
            <span className="text-[10px] text-ink-ghost">/ {maxRounds}</span>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col min-w-0">
            ...
            <div className="mx-auto max-w-2xl px-5 py-6">
              ...
              {messages.length === 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-32 text-center">
                  <div className="w-8 h-px bg-teal mx-auto mb-8" />
                  <p className="text-xl font-serif text-ink-ghost">El debate comenzará en breve</p>
          ...
          <div className="flex items-center gap-3 px-5 py-3 border-t border-rule bg-paper">
            {status !== "completed" && (
              <>
                <Button onClick={handleNextCycle} disabled={loading || status === "running"} className="group bg-ink text-paper hover:bg-teal-dark disabled:opacity-30 rounded px-5 text-xs font-medium uppercase tracking-wide">
                ...
                <Button variant="ghost" onClick={handleStop} disabled={status !== "running"} className="rounded text-ink-ghost hover:text-ink text-xs">
                  <Pause className="mr-1 h-3 w-3" /> Pausar
                </Button>
              </>
            )}
            <Button variant="ghost" onClick={handleExport} disabled={loading || messages.length === 0} className="ml-auto rounded text-ink-ghost hover:text-ink text-xs">
            ...
            >
              {config && (
                <div className="border border-rule rounded p-4">
                  <h3 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint mb-3">Panelistas</h3>
                    ...
                    {config.panelists.map((p) => (
                      <div key={p.name} className="flex items-center gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="text-[11px] font-medium text-ink block truncate">{p.name}</span>
